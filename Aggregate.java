...
public class Aggregate extends AbstractDbIterator {
    ...
    public Aggregate(DbIterator child, int afield, int gfield, Aggregator.Op aop) {
    	...
    	if(gfield==-1) {
        	...
        	types[0] = childtd.getType(afield);
        	td = new TupleDesc(types);
    	}
    	else {
    		Type[] types = new Type[2];
        	types[0] = childtd.getType(afield);
        	types[1] = childtd.getType(gfield);
        	td = new TupleDesc(types);
    	}
    }
    ...
    public void open()
    	...
    	if(child.hasNext()) {
    		next = child.next();
    		if(td.getType(0) == Type.INT_TYPE) {
    			if(gfield==-1)
    				ag = new IntAggregator(gfield, null, afield, aop);
    			else
    				ag = new IntAggregator(gfield, next.getField(gfield).getType(), afield, aop);
    		}
    		else {
    			if(gfield==-1)
    				ag = new StringAggregator(gfield, null, afield, aop);
    			else
    				ag = new StringAggregator(gfield, next.getField(gfield).getType(), afield, aop);
    		}
    		ag.merge(next);
    		while(child.hasNext()) {
    			next = child.next();
